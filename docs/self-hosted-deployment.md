...
is disabled so execution cannot attempt an install. `/tmp` retains root-owned
sticky permissions (`1777`) required by application-state custody validation.
It never dereferences a
